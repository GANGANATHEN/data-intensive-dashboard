...
export default function Dashboard() {
  ...
  if (isLoading)
    return <div className="p-10 text-center">Loading 10,000 records...</div>;
      ...
      >
        ...
        >
          {rowVirtualizer.getVirtualItems().map((virtualRow) => (
            <div
              key={virtualRow.key}
              className={`absolute top-0 left-0 w-full flex items-center px-4 border-b
               border-slate-100 ${
            ...
            >
              <span className="text-slate-400 w-12 font-mono text-xs">
                #{virtualRow.index}
              </span>
              ...
              <span
                className={`ml-auto px-2 py-0.5 rounded-full text-[10px] uppercase font-bold ${
                  data[virtualRow.index].status === "Active"
